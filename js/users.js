...
button.onclick = function() {
    if (!isLoading) {
        ...
        img.setAttribute("src", "https://c.tenor.com/I6kN-6X7nhAAAAAj/loading-buffering.gif");
        document.body.append(img);
        fetch(`https://jsonplaceholder.typicode.com/users?id=${Math.floor(Math.random() * 9 + 1)}`)
            .then(response => {
                if (response.ok) {
